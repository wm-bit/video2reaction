...
class ReactionPredictionModel(nn.Module):
    def __init__(self, embedding_dim=768, num_classes=28, hidden_dim=512):
    ...
    def _initialize_weights(self):
        """
        Apply Xavier initialization to the weights of all Linear layers.
        """
        for m in self.modules():
            if isinstance(m, nn.Linear):
                torch.nn.init.xavier_normal_(m.weight)  # Xavier normal initialization
                if m.bias is not None:
                    torch.nn.init.zeros_(m.bias)  # Initialize biases to zero

    def forward(self, text_embedding, visual_features, audio_features):
        """
        Args:
            text_embedding (Tensor): Shape (B, 768)
            visual_features (Tensor): Shape (B, 768)

        Returns:
            reaction_distribution (Tensor): Shape (B, 28)
        """
        text_out = self.text_fc(text_embedding)  # (B, hidden_dim)
        visual_out = self.visual_fc(visual_features)  # (B, hidden_dim)
        audio_out = self.audio_fc(audio_features)

        fused = torch.cat([text_out, visual_out, audio_out], dim=-1)  # (B, hidden_dim * 2)
        reaction_scores = self.fusion_fc(fused)  # (B, num_classes)
        reaction_distribution = F.softmax(reaction_scores, dim=-1)  # (B, num_classes)
        return reaction_distribution, reaction_scores
