...
from dataset.v2r_latent_set import SENTIMENT_2_FINER_GRAINED_MAPPING, SENTIMENT_CLASSES, SENTIMENT_2_FINER_GRAINED_INDICES_MAPPING, REACTION_INDEX
...
def compute_all_classification_metrics(
    ...
    # Multilabel metrics
    for p_threshold in [0.1, 0.2, 0.3]:
    ...
    if output_float:
        for key in results:
            if type(results[key]) != float and type(results[key]) != dict:
