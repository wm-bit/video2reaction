...
def get_v2r_pos_neg_batch_indices(label):
    """
    label: tensor of shape (B, 21) 
    return: two indices, one is including those batches with a positive donimnat emotion, the other is with the negative dominant emotion
    assert: length of the two indices should sum up tp B (batch size)
    """
    positive_mask = v2r_set.positive_mask.unsqueeze(0).to(label.device)
    negative_mask = v2r_set.negative_mask.unsqueeze(0).to(label.device)
    positive_expectation = (positive_mask * label).sum(dim=1)
    negative_expectation = (negative_mask * label).sum(dim=1)
    return positive_expectation > negative_expectation, negative_expectation >= positive_expectation
...
class MMILB(nn.Module):
    ...
    def forward(self, x, y, labels=None, mem=None):
        ...
        lld = torch.mean(torch.sum(positive,-1))  # optimize the mu and var mlp model so that the gaussian conditioned on modality X using the two mlp has a high likelihood on modality Y
        ...
        if labels is not None:
            ...
            if len(labels.squeeze().shape) == 1:
                ...
                pos_batch, neg_batch = get_v2r_pos_neg_batch_indices(labels)
                pos_y = y[pos_batch]
                neg_y = y[neg_batch]
            ...
            if mem is not None and mem.get('pos', None) is not None:
                ...
                a = 17.0795
                # print(f"mminfomax.modules.encoders.MMILB: inp_min_eigenvalue = {torch.linalg.eigvals(sigma_pos).real.min(), torch.linalg.eigvals(sigma_neg).real.min()},  sigma_pos = {sigma_pos.shape}, sigma_neg = {sigma_neg.shape}, size_pos = {pos_all.shape}, size_neg = {neg_all.shape}")
                # H = 0.25 * (torch.logdet(sigma_pos) + torch.logdet(sigma_neg))
                H = 0.25 * (torch.slogdet(sigma_pos).logabsdet + torch.slogdet(sigma_neg).logabsdet)
